...
char *get_username(void)
{
    ...
    tuser = snewn(namelen, TCHAR);
	GetUserName(tuser, &namelen);

#if defined(UNICODE) || defined(_UNICODE)
	{
		size_t sz = wcslen(tuser);
		char *user = snewn(sz+1, char);
		WideCharToMultiByte(CP_UTF8, 0, tuser, sz, user, sz, NULL, NULL);
		user[sz] = 0;
		sfree(tuser);
		return user;
	}
