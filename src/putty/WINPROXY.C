			       ...
			       Plug plug, const Config *cfg)
{
	...
	{
		char *bcmd = format_telnet_command(addr, port, cfg);
		size_t sz = strlen(bcmd);
		cmd = snewn(sz+1, TCHAR);
		MultiByteToWideChar(CP_ACP, 0, bcmd, sz, cmd, sz);
		cmd[sz] = 0;
	}
#else
