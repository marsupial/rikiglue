...
/*
 * Various remote-bug ssh->cfg.flags.
 */
			 ...
			 struct Packet *pktin)
{
	...
	if (!*ssh->cfg.username) {
	    ...
	    add_prompt(s->cur_prompt, dupstr("login as: "), TRUE,
		       lenof(s->username)); 
	    ret = get_userpass_input(s->cur_prompt, NULL, 0, &ssh->cfg);
	    while (ret < 0) {
		ssh->send_ok = 1;
		crWaitUntil(!pktin);
		ret = get_userpass_input(s->cur_prompt, in, inlen, &ssh->cfg);
		ssh->send_ok = 0;
	    }
	    if (!ret) {
		/*
	...
	send_packet(ssh, SSH1_CMSG_USER, PKT_STR, s->username, PKT_END);
	{
	    ...
	    logevent(userlog);
	    if (ssh->cfg.flags & FLAG_INTERACTIVE &&
		(!((ssh->cfg.flags & FLAG_STDERR) && (ssh->cfg.flags & FLAG_VERBOSE)))) {
			     ...
			     struct Packet *pktin)
{
	...
	if (s->got_username && !ssh->cfg.change_username) {
	    ...
	    strncpy(s->username, ssh->cfg.username, sizeof(s->username));
	    s->username[sizeof(s->username)-1] = '\0';
	    if ((ssh->cfg.flags & FLAG_VERBOSE) || (ssh->cfg.flags & FLAG_INTERACTIVE)) {
	...
	while (1) {
		 ...
		 * output of (say) plink.)
		 */
		if (size && (ssh->cfg.flags & (FLAG_VERBOSE | FLAG_INTERACTIVE))) {
		...
		if (pktin->type != SSH2_MSG_USERAUTH_PK_OK) {
		    ...
		    *s->q++ = SSH2_AGENTC_SIGN_REQUEST;
		    ...
		    /* And finally the (zero) ssh->cfg.flags word. */
		    PUT_32BIT(s->q, 0);
